...
class Flux2Model(BaseModel):
    ...
    def load_te(self):
        ...
        # Check for local text_encoder directory first
        local_te_path = os.path.join(self.model_config.name_or_path, "text_encoder")
        if os.path.exists(local_te_path):
            self.print_and_status_update(f"Loading text encoder from local path: {local_te_path}")
            text_encoder_path = local_te_path
        else:
    ...
    def load_model(self):
        ...
        target_device = "cpu" if self.model_config.low_vram else self.device_torch
    ...
    def _resolve_transformer_path(self, model_path: str) -> str:
        """按优先级查找 transformer 权重：根目录单文件 > transformer/ 子目录 > HuggingFace Hub。"""
        candidates = [
            os.path.join(model_path, self.flux2_te_filename),
        ...
        self.print_and_status_update("Downloading transformer checkpoint from Hub")
    ...
    def _load_transformer(self, transformer_path: str, dtype: torch.dtype) -> Flux2:
        """构建、加载并配置 Flux2 Transformer。"""
        ...
        if self.model_config.quantize:
            ...
            self.print_and_status_update("Quantizing Transformer (keeping on CPU)")
        ...
        if self.model_config.low_vram:
            self.print_and_status_update("Offloading transformer to CPU")
            transformer.to("cpu")

        return transformer


    def _load_vae(self, model_path: str, dtype: torch.dtype):
        """自动检测并加载 VAE（diffusers 目录格式 或 单文件原生格式）。"""
        self.print_and_status_update("Loading VAE")

        vae_path = self.model_config.vae_path

        # 自动识别标准 diffusers 布局下的 vae 子目录
        diffusers_dir = os.path.join(model_path, "vae")
        if os.path.isdir(diffusers_dir) and os.path.exists(os.path.join(diffusers_dir, "config.json")):
            vae_path = diffusers_dir

        if vae_path is None:
            vae_path = self.flux2_vae_path

        if os.path.isdir(vae_path) and os.path.exists(os.path.join(vae_path, "config.json")):
            return self._load_vae_diffusers(vae_path, dtype)

        return self._load_vae_single_file(vae_path, model_path, dtype)


    def _load_vae_diffusers(self, vae_path: str, dtype: torch.dtype):
        """加载 diffusers 格式 VAE，并包装 encode/decode 以对齐原生 latent 排列。"""
        vae = AutoencoderKLFlux2.from_pretrained(vae_path, torch_dtype=dtype)
        p1, p2 = vae.config.patch_size

        _orig_encode = vae.encode
        _orig_decode = vae.decode

        def _compat_encode(x):
            z = _orig_encode(x).latent_dist.sample()
            return rearrange(z, "b c (h p1) (w p2) -> b (c p1 p2) h w", p1=p1, p2=p2)

        def _compat_decode(z):
            z = rearrange(z, "b (c p1 p2) h w -> b c (h p1) (w p2)", p1=p1, p2=p2)
            return _orig_decode(z).sample

        vae.encode = _compat_encode
        vae.decode = _compat_decode
        return vae


    def _load_vae_single_file(self, vae_path: str, model_path: str, dtype: torch.dtype):
        """加载 BFL 原生单文件格式 VAE。"""
        if not os.path.exists(vae_path):
            vae_filename = FLUX2_VAE_FILENAME
            if vae_path is not None:
                parts = vae_path.split("/")
                if len(parts) == 3 and vae_path.endswith(".safetensors"):
                    vae_filename = parts[-1]
                    vae_path = "/".join(parts[:-1])
            repo = vae_path if vae_path is not None else model_path
            vae_path = huggingface_hub.hf_hub_download(
                repo_id=repo, filename=vae_filename, token=HF_TOKEN,
            )
        ...
        bias_key = "decoder.up.0.block.0.conv1.bias"
        if bias_key in state_dict and state_dict[bias_key].shape[0] == 96:
